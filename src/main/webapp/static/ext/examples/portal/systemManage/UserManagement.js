...
Ext.onReady(function() {
	...
	Ext.define('Stuattd.app.systemManage.UserManagement.Import',{
	    ...
	    items:[/*{
            ...
            vertical: true,
            items:[
                   { boxLabel: '学生信息', name: 'rb', inputValue: 'student', checked: true },
                   { boxLabel: '教师信息', name: 'rb', inputValue: 'teacher'},
                   { boxLabel: '班级信息', name: 'rb', inputValue: 'class'}
            ]
	    },*/{
			...
			items : [ {
				...
				html : '2、学生表模板<a href="' + appBaseUri + '/sys/ClassManagement/downloadImportedFile?name=student")>点此下载.</a>'
			},{
				html : '3、教师表模板<a href="' + appBaseUri + '/sys/ClassManagement/downloadImportedFile?name=teacher")>点此下载.</a>'
			}  ]
		} ],
	    buttons: [{  
            text: '保存',  
            handler: function() {  
                if (Ext.getCmp('fp').isValid()) {  
                	//alert(Ext.getCmp('form-file').value);
                	//alert(Ext.getCmp('fp').getForm().findField('rb').inputValue);
                	Ext.getCmp('fp').submit({  
                        url: appBaseUri + '/sys/fileUpload/getFile',//后台处理的页面  
                        params:Ext.getCmp('fp').form.getValues(),
            ...
            text: '重置',  
            handler: function() {  
            	Ext.getCmp('fp').form.reset();  
            }  
}]
	});
	
	Ext.define('Stuattd.app.systemManage.UserManagement', {
