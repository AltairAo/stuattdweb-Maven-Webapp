...
Ext.onReady(function() {
	...
	Ext.define('App.StuattdEntryWindow', {
		...
		constructor : function(config) {
					...
					items : [ {
						...
						listeners : {
							select : function(combo, record, index) {
								Ext.getCmp("stuattdEntryForm-stuattdTypeId").setValue(combo.getValue());
							}
						}
					}, {
						xtype : 'hiddenfield',
						id : 'stuattdEntryForm-stuattdTypeId',
						name : 'stuattdTypeId'
					} ],
					buttons : [ '->', {
						id : 'stuattdentrywindow-save',
						text : '保存',
						iconCls : 'icon-save',
						width : 80,
									...
									success : function(response) {
										if (response.responseText != '') {
											...
											if (res.success) {
												...
												Ext.getCmp('stuattdentrygrid').getStore().reload();
											} else {
			...
			App.StuattdEntryWindow.superclass.constructor.call(this, config);
		}
	});

	Ext.define('Stuattd.app.stuattdManage.StuattdEntry', {
		...
		initComponent : function() {
			...
			Ext.define('ModelList', {
				...
				fields : [ {
					...
					type : 'datetime',
					dateFormat : 'Y-m-d H:i:s'
				}, 'stuattdTypeName', 'stuattdTypeId', 'stuattdTypeImagePath' ]
			});
			...
			var columns = [ {
				...
				text : "图片路径",
				dataIndex : 'stuattdTypeImagePath',
				hidden : true,
				sortable : false
			}, {
				xtype : 'actioncolumn',
				...
				items : [ {
					...
					handler : function(grid, rowIndex, colIndex) {
						...
						new Ext.window.Window({
							...
							autoScroll : true,
							items : [ {
								html : entity.get('stuattdTypeImagePath')
							} ]
						}).show();
					}
				}, {
					...
					handler : function(grid, rowIndex, colIndex) {
						...
						var win = new App.StuattdEntryWindow({
							hidden : true
						});
						...
						Ext.getCmp('stuattdentrywindow-save').hide();
						Ext.getCmp('stuattdentrywindow-cancel').hide();
						Ext.getCmp('stuattdentrywindow-accept').show();
						win.show();
					}
				}, {
					iconCls : 'edit',
		...
		onAddClick : function() {
			new App.StuattdEntryWindow().show();
		},
		onViewClick : function() {
			var grid = Ext.getCmp("stuattdentrygrid");
