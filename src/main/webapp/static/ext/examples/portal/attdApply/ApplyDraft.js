...
Ext.define('Stuattd.app.attdApply.ApplyDraft', {
	...
	initComponent : function() {
		if (mainTab.getComponent('tab26')) {
			mainTab.getComponent('tab26').close();
		}
		var me = this;
		...
		var store = Ext.create('Ext.data.Store', {
			...
			proxy : {
				type : 'ajax',
				url : appBaseUri + '/attd/attdDraft/getQjxx?xsjbxxId='+xsjbxxId,
		...
		var ttoolbar = Ext.create('Ext.toolbar.Toolbar', {
			items : [ {
				...
				handler : function(btn, eventObj) {
					var searchParams = {
						param : Ext.getCmp('applydraft-xsjbxxId').getValue(),
						kssj : Ext.util.Format.date(Ext.getCmp('applydraft-kssjquery').getValue(),'Y-m-d'),
						jssj : Ext.util.Format.date(Ext.getCmp('applydraft-jssjquery').getValue(),'Y-m-d')
				...
				maxWidth : 60,
				handler : function(btn, eventObj) {
					Ext.getCmp('applydraft-xsjbxxId').setValue(null);
					Ext.getCmp('applydraft-kssjquery').setValue(null);
					Ext.getCmp('applydraft-jssjquery').setValue(null);
				}
			} ]
		});
		Ext.apply(this, {
