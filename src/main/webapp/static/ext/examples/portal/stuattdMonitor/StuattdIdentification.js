 ...
 */
// 树木位置标识
Ext.define('Stuattd.app.stuattdMonitor.StuattdIdentification', {
	extend : 'Ext.panel.Panel',
	initComponent : function() {
		var me = this;
		Ext.apply(this, {
			layout : 'border',
			items : [ Ext.create('Stuattd.app.stuattdMonitor.SensorLeaflet'), Ext.create('Stuattd.app.stuattdMonitor.SensorGrid', {
...
// 传感器在Leaflet地图的位置标识
Ext.define('Stuattd.app.stuattdMonitor.SensorLeaflet', {
	extend : 'Ext.panel.Panel',
	region : 'north',
	height : '50%',
	split : true,
	html : '<iframe src= "' + appBaseUri + '/static/leaflet/StuattdIdentification.html" width="100%" height="100%" marginwidth="0" framespacing="0" marginheight="0" frameborder="0" ></iframe>',
	initComponent : function() {
		this.callParent(arguments);
	}
});

Ext.define('App.StuattdIdentificationChoiceWindow', {
	...
	constructor : function(config) {
				...
				buttons : [ '->', {
					...
					handler : function(btn, eventObj) {
						...
						if (choiceradiogroup == true) {
							...
							var win = new App.StuattdIdentificationWindow({
								hidden : true
							});
		...
		App.StuattdIdentificationChoiceWindow.superclass.constructor.call(this, config);
	}
});

Ext.define('App.StuattdIdentificationWindow', {
	extend : 'Ext.window.Window',
	constructor : function(config) {
		config = config || {};
		Ext.apply(config, {
								...
								success : function(response) {
									if (response.responseText != '') {
										...
										if (res.success) {
											...
											Ext.getCmp('stuattdidentificationgrid').getStore().reload();
										} else {
		...
		App.StuattdIdentificationWindow.superclass.constructor.call(this, config);
	}
});

Ext.define('Stuattd.app.stuattdMonitor.SensorGrid', {
	...
	onAddClick : function() {
		new App.StuattdIdentificationChoiceWindow().show();
	},
	onDeleteClick : function() {
		globalObject.confirmTip('删除的记录不可恢复，继续吗？', function(btn) {
			if (btn == 'yes') {
				var s = Ext.getCmp("stuattdidentificationgrid").getSelectionModel().getSelection();
				if (s == "") {
